...
class Net(nn.Module):
    def __init__(self, state_dim=8, action_dim=4, hidden_dim=32, device='cuda'):
        super().__init__()
        ## TODO ##
        self.device = device
...
class DQN:
    def __init__(self, args):
        self._behavior_net = Net(device=args.device).to(args.device)
        self._target_net = Net(device=args.device).to(args.device)
    ...
    def _update_target_network(self, tau=1e-3):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                break
        # early stopping
        reward_queue_total += total_reward
        if reward_queue.full():
            reward_queue_total -= reward_queue.get()
        reward_queue.put(total_reward)
        if (reward_queue_total/100) > 200:
            break
